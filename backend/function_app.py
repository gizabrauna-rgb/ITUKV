...
def ausschreibung_versand(req: func.HttpRequest) -> func.HttpResponse:
    ...
    for r in targets_list:
        ...
        try:
            ...
            # Reply-To pro Empfaenger: kontakt-{token}@reply.itukv.de
            # So koennen Antworten beim Inbound-Webhook dem Kontakt zugeordnet werden.
            msg = {
                "senderAddress": ACS_SENDER,
            ...
            try:
                ...
                token = _create_reply_token(tid, r.get("email", ""))
                msg["replyTo"] = [{"address": f"kontakt-{token}@{reply_domain}", "displayName": "Jennifer Kaplan"}]
            except Exception: pass
...
def _parse_sendgrid_multipart(req) -> dict:
    """Parst multipart/form-data von SendGrid mit korrektem Charset-Handling."""
    from email import message_from_bytes
    from email.policy import compat32
    ct = req.headers.get("Content-Type", "") or ""
    if not ct.lower().startswith("multipart/"):
        return {}
    raw = req.get_body() or b""
    mime_bytes = b"Content-Type: " + ct.encode("ascii", "ignore") + b"\r\n\r\n" + raw
    msg = message_from_bytes(mime_bytes, policy=compat32)
    raw_fields = {}
    for part in msg.walk():
        ...
        raw_fields[name] = (payload, part.get_content_charset())
    charsets = {}
    cs_raw = raw_fields.get("charsets", (b"{}", None))[0]
    if cs_raw:
        try: charsets = json.loads(cs_raw.decode("ascii", errors="replace") or "{}")
        except Exception: charsets = {}
    def _dec(payload, fallback_cs, field_cs):
        if not payload: return ""
        for cs in (field_cs, fallback_cs, "utf-8", "windows-1252", "iso-8859-1"):
            if not cs: continue
            try: return payload.decode(cs)
            except (UnicodeDecodeError, LookupError): continue
        return payload.decode("utf-8", errors="replace")
    result = {}
    for name, (payload, part_cs) in raw_fields.items():
        if payload is None: result[name] = ""; continue
        result[name] = _dec(payload, part_cs, charsets.get(name))
    return result


def _create_reply_token(target_id, kontakt_email):
    """Erzeugt einen kurzen Reply-Token + speichert ihn fuer Lookup beim Inbound."""
    token = secrets.token_urlsafe(8).lower().replace("-", "").replace("_", "")[:12]
    try:
        table_("replytokens").create_entity({
            "PartitionKey": "rt",
            "RowKey": token,
            "targetId": target_id or "",
            "kontaktEmail": (kontakt_email or "").lower(),
            "createdAt": datetime.utcnow().isoformat(),
        })
    except Exception as ex:
        logging.warning(f"Reply-Token speichern fehlgeschlagen: {ex}")
    return token


@app.route(route="inbound-mail", methods=["POST", "GET"], auth_level=func.AuthLevel.ANONYMOUS)
def inbound_mail(req: func.HttpRequest) -> func.HttpResponse:
    """Empfaengt SendGrid Inbound Parse Webhook und legt die Mail im
    Kontakt-Verlauf ab + leitet sie an Jenny weiter.
    Authentifiziert ueber ?key=... gegen ENV INBOUND_MAIL_KEY."""
    expected_key = os.environ.get("INBOUND_MAIL_KEY", "")
    provided_key = req.params.get("key", "")
    if expected_key and provided_key != expected_key:
        return func.HttpResponse("forbidden", status_code=403)
    if req.method == "GET":
        return func.HttpResponse("inbound-mail endpoint ok", status_code=200)
    try:
        form = _parse_sendgrid_multipart(req)
        if not form:
            form = {k: v for k, v in (req.form or {}).items()}
        to_field = (form.get("to") or "").strip()
        ...
        try: envelope = json.loads(envelope_raw) if envelope_raw else {}
        except Exception: envelope = {}

        # Token aus to-Adresse extrahieren: kontakt-{token}@reply.itukv.de
        recipients = []
        et = envelope.get("to")
        if isinstance(et, list): recipients.extend(et)
        elif isinstance(et, str): recipients.append(et)
        if to_field: recipients.append(to_field)
        token = None
        pat = re.compile(r"kontakt-([a-z0-9]+)@", re.IGNORECASE)
        for r in recipients:
            m = pat.search(r or "")
            if m: token = m.group(1).lower(); break

        # Absender-E-Mail aus from-Feld extrahieren (oft "Name <addr>")
        from_email = ""
        ...
        if token:
            try:
                rec = table_("replytokens").get_entity("rt", token)
                target_id = rec.get("targetId", "") or ""
                if rec.get("kontaktEmail"): kontakt_email = rec["kontaktEmail"]
            except Exception:
                logging.warning(f"Reply-Token {token} nicht gefunden")

        now_iso = datetime.utcnow().isoformat()

        # 1) Eintrag im Kontakt-Verlauf (per Email-Match)
        if kontakt_email:
            try:
                tc = table_("kontakte")
                kontakt = None
                ...
                if kontakt:
                    ...
                    kverlauf.append({
                        ...
                        "beschreibung": body_text or re.sub(r"<[^>]+>", " ", body_html or "")[:4000],
        ...
        # 2) Mail an Jenny weiterleiten (mit klarem Subject-Prefix)
        if ACS_CONN:
            try:
                from azure.communication.email import EmailClient
                client = EmailClient.from_connection_string(ACS_CONN)
                ...
                fwd_html = (
                    '<html><body style="font-family:Arial,sans-serif;color:#161e2a;line-height:1.5">'
                    f'<p style="background:#eaf6fa;padding:10px;border-radius:6px;font-size:12px;color:#0088ba">'
                    f'<strong>Antwort auf Ausschreibung</strong> — automatisch ins Dashboard übernommen.<br/>'
                    f'Von: <strong>{from_field}</strong>'
                    f'{(" · Mandat: " + target_id) if target_id else ""}'
                    f'</p>'
                    + (body_html or f'<pre>{body_text}</pre>')
                    + '</body></html>'
                )
                client.begin_send({
                    "senderAddress": ACS_SENDER,
                    ...
                    "content": {
                        "subject": f"[ITUKV] Antwort: {subject or from_email}",
                        "plainText": f"Antwort von {from_field}\n\nBetreff: {subject}\n\n{body_text}",
                        "html": fwd_html,
                    },
                    "replyTo": [{"address": from_email}] if from_email else None,
                })
            except Exception as ex:
        ...
        logging.error(f"inbound-mail Fehler: {e}")
        return func.HttpResponse("error", status_code=200)  # 200 damit SendGrid nicht retried
